...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (scribeError || !scribe) {
      ...
      return new Response(JSON.stringify({ error: "Scribe not found" }), {
        status: 404,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (documentIds.length === 0) {
      console.log("No documents found in scribe");
      return new Response(
        JSON.stringify({
          message:
            "This scribe doesn't have any documents yet. Please save some documents to this scribe first.",
          tokens: 0,
          contextChunks: 0,
          sources: [],
        }),
    ...
    console.log(
      `🔍 Checking for vectors in ${documentIds.length} documents:`,
      documentIds.slice(0, 5)
    );
    const {
      data: sampleVectors,
      count: vectorCount,
      ...
      .select(
        "embedding, metadata, embedding_provider, embedding_dimensions, document_id",
        { count: "exact" }
      )
    ...
    if (sampleVectors && sampleVectors.length > 0) {
      console.log(
        "📊 Sample vector document_id:",
        sampleVectors[0].document_id
      );
      console.log(
        "📊 Sample vector has embedding:",
        !!sampleVectors[0].embedding
      );
    } else {
      console.warn("⚠️ No vectors found. This could mean:");
      console.warn(
        "   1. Documents have not been processed for vector storage"
      );
    ...
    if (sampleVectors && sampleVectors.length > 0) {
      ...
      console.log("📐 Expected dimensions: 1536 (OpenAI text-embedding-3-small)");

      // Check for dimension mismatch
      if (storedDim && storedDim !== queryEmbedding.length) {
        console.error("❌ DIMENSION MISMATCH!");
        console.error(`   Query: ${queryEmbedding.length} dimensions (OpenAI text-embedding-3-small)`);
        console.error(`   Stored: ${storedDim} dimensions`);
        console.error(`   Expected: 1536 dimensions`);

        return new Response(
          JSON.stringify({
            message: `Embedding dimension mismatch detected. Your documents were processed with ${storedDim} dimensions, but the query is using ${queryEmbedding.length} dimensions (OpenAI text-embedding-3-small). Please reprocess your documents to ensure they use the same embedding model.`,
            tokens: 0,
            contextChunks: 0,
            sources: [],
    ...
    if (!vectorCount || vectorCount === 0) {
      console.warn(
        "⚠️ No vectors found for user. Documents may not have been processed."
      );
      // Return helpful message
      return new Response(
        JSON.stringify({
          message:
            "I don't have any information from your documents yet. Please save some documents first, and make sure they are processed for vector storage. You can check if documents have been processed in the Supabase dashboard.",
          tokens: 0,
          contextChunks: 0,
          sources: [],
          warning:
            "No document vectors found. Documents may need to be processed.",
        }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    console.log(
      "🔍 Performing semantic search with threshold:",
      matchThreshold
    );
    console.log("🔍 Query embedding dimensions:", queryEmbedding.length);
    ...
    console.log(
      `🔍 Direct vector query test: Found ${
        testCount || 0
      } vectors accessible via RLS`
    );
    if (testVectors && testVectors.length > 0) {
      console.log(
        "✅ Vectors are accessible! Sample document_ids:",
        testVectors.map((v) => v.document_id)
      );
    } else {
      console.warn(
        "⚠️ Direct vector query returned 0 results - RLS might be blocking or vectors don't exist"
      );
    }
    ...
    console.log(
      `🔍 Searching with scribe_filter: ${scribeId}, documentIds: ${documentIds.length} documents`
    );
    const { data: flexibleResults, error: flexibleError } =
      await supabaseClient.rpc("search_documents_semantic", {
    ...
    if (flexibleResults) {
      console.log(
        `📊 Flexible search returned ${flexibleResults.length} results`
      );
      const resultDocIds = [
        ...new Set(flexibleResults.map((r: any) => r.document_id)),
      ];
      console.log(
        `📊 Results from ${resultDocIds.length} unique documents:`,
        resultDocIds
      );
      const invalidDocs = resultDocIds.filter(
        (id) => !documentIds.includes(id)
      );
      if (invalidDocs.length > 0) {
        console.warn(
          `⚠️ Found ${invalidDocs.length} documents not in scribe:`,
          invalidDocs
        );
      }
    }
    ...
    if (flexibleError) {
      ...
      const { data: specificResults, error: specificError } =
      ...
      if (specificError) {
        console.error(
          "❌ Dimension-specific search also failed:",
          specificError
        );
        console.error(
        ...
        try {
          const { data: directResults, error: directError } =
            await supabaseClient
              .from("document_vector")
              .select(
                "id, document_id, chunk_text, page_number, section_title, chunk_index, embedding"
              )
              .in("document_id", documentIds)
              .not("embedding", "is", null)
              .limit(matchCount);
          ...
          if (directError) {
            ...
            console.log(
              "⚠️ Direct query returned",
              directResults?.length || 0,
              "vectors (no similarity calculation)"
            );
            ...
            if (directResults && directResults.length > 0) {
              ...
              console.log(
                "⚠️ Using direct query results with placeholder similarity"
              );
            }
          }
        } catch (directErr) {
    ...
    if (searchError) {
      ...
      if (
        searchError.message?.includes("dimension") ||
        searchError.message?.includes("vector")
      ) {
        throw new Error(
          `Embedding dimension mismatch. Query embedding has ${queryEmbedding.length} dimensions (OpenAI text-embedding-3-small), but stored vectors may have different dimensions. Please reprocess your documents with the OpenAI embedding model.`
        );
      }
      throw searchError;
    }
    ...
    if ((!searchResults || searchResults.length === 0) && vectorCount > 0) {
      console.log(
        "⚠️ No results with threshold",
        matchThreshold,
        "- trying lower threshold 0.5"
      );
      ...
      const { data: lowerThresholdResults, error: lowerError } =
        await supabaseClient.rpc(functionName, {
          query_embedding: queryEmbedding,
          match_threshold: 0.5,
          match_count: matchCount,
      ...
      if (lowerError) {
        ...
        const { data: veryLowResults } = await supabaseClient.rpc(
          functionName,
          {
            query_embedding: queryEmbedding,
            match_threshold: 0.3,
        ...
        if (veryLowResults && veryLowResults.length > 0) {
          console.log(
            "✅ Found",
            veryLowResults.length,
            "results with very low threshold"
          );
          ...
          const { data: anyResults } = await supabaseClient.rpc(functionName, {
            query_embedding: queryEmbedding,
            match_threshold: 0.0,
            match_count: matchCount,
            user_filter: null, // RLS will handle access control
          ...
          if (anyResults && anyResults.length > 0) {
            console.log(
              "✅ Found",
              anyResults.length,
              "results with minimum threshold"
            );
            searchResults = anyResults;
          } else {
            console.error(
              "❌ No results found even with threshold 0.0 - possible dimension mismatch or empty vectors"
            );
          }
        }
      }
    ...
    if (searchResults && searchResults.length > 0) {
      ...
      searchResults = searchResults.filter((result: any) => {
        ...
        if (!isValid) {
          console.warn(
            `⚠️ Filtered out chunk from document ${result.document_id} (not in scribe ${scribeId})`
          );
        }
        return isValid;
      });
      ...
      if (searchResults.length < originalCount) {
        console.warn(
          ...
          } chunks that didn't belong to scribe ${scribeId}`
        );
      }

      console.log(
        `✅ Validated ${searchResults.length} chunks belong to scribe ${scribeId}`
      );
    }
      ...
      contextString = searchResults
        ...
        .map((result: any, index: number) => {
          ...
          if (result.section_title)
            part += `Section: ${result.section_title}\n`;
          if (result.page_number) part += `Page: ${result.page_number}\n`;
          if (result.similarity)
            part += `Relevance: ${(result.similarity * 100).toFixed(1)}%\n`;
...
- Be helpful and friendly.`);
    ...
    function getOpenAIModel(scribeModel: string | null | undefined): string {
      ...
      if (groqModels.includes(scribeModel)) {
        console.log(
          `⚠️ Scribe model "${scribeModel}" is a Groq model, using default OpenAI model`
        );
        return "gpt-4-turbo-preview";
      }
    ...
    // Save AI response
    await supabaseClient.from("scribe_message").insert({
      scribe_id: scribeId,
      user_id: user.id,
      role: "assistant",
      content: aiResponse,
    ...
    const sourceDocumentIds = [
      ...new Set(searchResults?.map((r: any) => r.document_id) || []),
    ];
